...
//
// It lives in @shared (pure string templates, no Node deps) so BOTH the server
// and the browser can render. That's what makes theme changes instant — the
// client recomputes CSS locally with zero network and zero AI.
// ============================================================================
import type { SiteDocument, Section, Theme, SectionType } from "./site-document";
...
/** Google Fonts href for a theme (used to inject the right fonts in preview). */
export function fontHref(theme: Theme): string {
  return `https://fonts.googleapis.com/css2?family=${tokens(theme).fontImport}&display=swap`;
}
...
/* skeleton (two-phase: shown while section copy is being written) */
.skel-block { background:var(--surface); border:1px solid var(--border); border-radius:var(--r); }
.skel-line { height:.85rem; border-radius:var(--r); background:var(--border); opacity:.6; margin:.6rem 0; }
.skel-line.w-40 { width:40%; } .skel-line.w-60 { width:60%; } .skel-line.w-80 { width:80%; }
.skel-pulse { animation:skelPulse 1.1s ease-in-out infinite; }
@keyframes skelPulse { 0%,100% { opacity:.55; } 50% { opacity:.9; } }
.skel-card { padding:2rem; min-height:9rem; }
...
export function renderDocumentBody(doc: SiteDocument): string {
  const sections = doc.sections.map((s) => renderSection(s, doc)).join("\n");
  return `${sections}
...
// ============================================================================
// Two-phase generation: a lightweight OUTLINE (structure only) rendered as a
// themed skeleton the instant phase 1 returns, before section copy is written.
// ============================================================================
...
export interface SiteOutline {
  meta: { name: string; tagline?: string; industry?: string };
  theme: Theme;
  outline: OutlineSection[];
}

function skelLines(widths: string[]): string {
  return widths.map((w) => `<div class="skel-line skel-pulse ${w}"></div>`).join("");
}

// A shimmer placeholder shaped roughly like each real section type, so the
// skeleton already conveys layout while the model writes the copy.
function renderSkeletonSection(s: OutlineSection, idx: number): string {
  const alt = idx % 2 === 1 ? "band-alt" : "";
  if (s.type === "hero") {
    return `
  <section class="hero">
    <div class="wrap hero-inner">
      <div class="skel-line skel-pulse w-40"></div>
      <div class="skel-line skel-pulse w-80" style="height:3rem;"></div>
      <div class="skel-line skel-pulse w-60"></div>
    </div>
  </section>`;
  }
  const cards = s.type === "services" || s.type === "products" || s.type === "testimonials" || s.type === "gallery";
  const body = cards
    ? `<div class="grid grid-3">${[0, 1, 2]
        .map(() => `<div class="skel-block skel-card skel-pulse">${skelLines(["w-60", "w-80", "w-40"])}</div>`)
        .join("")}</div>`
    : `<div class="skel-block skel-card skel-pulse">${skelLines(["w-80", "w-80", "w-60"])}</div>`;
  return `
...
/** CSS for the skeleton — same theme machinery as the real render. */
export function renderOutlineCss(outline: SiteOutline): string {
  return renderCss(outline.theme);
}

/** Body HTML for the themed skeleton shown between phase 1 and phase 2. */
export function renderOutlineBody(outline: SiteOutline): string {
  const sections = outline.outline.map((s, i) => renderSkeletonSection(s, i)).join("\n");
  return `${sections}
  <footer class="site-footer">
    <div class="wrap">© ${esc(outline.meta.name)}</div>
  </footer>`;
}
